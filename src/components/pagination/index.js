...
const CustomPagination = ({
  ...
  const previousPagesClasses =
    previousPagesClassName +
    (currentSelected < pageRange ? ` ${disabledClassName}` : "");
  ...
  const previousPagesAriaDisabled =
    currentSelected < pageRange ? "true" : "false";
  const nextPagesAriaDisabled =
    currentSelected >= pageCount - pageRange ? "true" : "false";
  ...
  const pagination = () => {
    ...
    if (pageCount <= pageRangeDisplayed) {
      ...
      for (index = 0; index < pageCount; index++) {
        ...
        if (
          index >= currentSelected - leftSide &&
          index <= currentSelected + rightSide
        ) {
          items.push(createPageView(index));
          continue;
        }
